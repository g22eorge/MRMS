...
const base = (body: string) => `
  <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#0d0d0d;color:#e5e5e5;border-radius:12px">
    <div style="margin-bottom:24px">
      <span style="font-size:11px;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#C9A227">Duuka ProMax</span>
    </div>
    ${body}
...
export async function sendWelcomeEmail(to: string, name: string, orgName: string) {
  ...
  await send(to, `Welcome to Duuka ProMax — ${esc(orgName)}`, base(`
    ...
    <a href="${appUrl}/dashboard" style="display:inline-block;background:#C9A227;color:#000;font-weight:700;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none">Open your workspace →</a>
  `));
}
...
export async function sendTrialExpiryWarning(to: string, name: string, orgName: string, daysLeft: number) {
  ...
  await send(to, `Your trial ends in ${daysLeft} day${daysLeft !== 1 ? "s" : ""} — ${esc(orgName)}`, base(`
    ...
    <p style="color:#aaa;line-height:1.6;margin:0 0 24px">Your free trial for <strong style="color:#fff">${esc(orgName)}</strong> ends in <strong style="color:#C9A227">${daysLeft} day${daysLeft !== 1 ? "s" : ""}</strong>. Upgrade to keep your data and continue managing repairs.</p>
    <a href="${appUrl}/settings/billing" style="display:inline-block;background:#C9A227;color:#000;font-weight:700;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none">Upgrade now →</a>
  `));
}
...
export async function sendPaymentConfirmation(to: string, name: string, orgName: string, plan: string, amount: number) {
  await send(to, `Payment confirmed — ${esc(orgName)}`, base(`
    ...
    <div style="background:#1a1a1a;border-radius:8px;padding:16px;margin-bottom:24px">
      ...
      <p style="margin:4px 0 0;font-size:24px;font-weight:700;color:#C9A227">UGX ${amount.toLocaleString()}</p>
    </div>
...
export async function sendPaymentFailedAlert(to: string, name: string, orgName: string) {
  ...
  await send(to, `Action required: payment failed — ${esc(orgName)}`, base(`
    ...
    <a href="${appUrl}/settings/billing" style="display:inline-block;background:#C9A227;color:#000;font-weight:700;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none">Update payment →</a>
  `));
}
