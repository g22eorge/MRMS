  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 px-4 py-6">
      ...
      <form action={createAnnouncementAction} className="space-y-3 rounded-xl border border-[var(--line)] bg-[var(--panel)] p-4">
        <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)]">Publish new</p>
        <input name="title" required placeholder="Title" className="w-full rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-[0.8125rem] outline-none focus:border-[var(--accent)]/50" />
        <textarea name="body" required rows={3} placeholder="Message shown to all organizations…" className="w-full rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-[0.8125rem] outline-none focus:border-[var(--accent)]/50" />
        <div className="grid gap-2 sm:grid-cols-3">
          <label className="text-[0.75rem] text-[var(--ink-muted)]">
            Level
            <select name="level" defaultValue="INFO" className="mt-1 w-full rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-[0.8125rem]">
          ...
          <label className="text-[0.75rem] text-[var(--ink-muted)]">
            Starts (optional)
            <input type="datetime-local" name="startsAt" className="mt-1 w-full rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-[0.8125rem]" />
          </label>
          <label className="text-[0.75rem] text-[var(--ink-muted)]">
            Ends (optional)
      ...
      <div className="space-y-2">
        {announcements.length === 0 ? (
          <p className="rounded-xl border border-dashed border-[var(--line)] py-10 text-center text-[0.8125rem] text-[var(--ink-muted)]">No announcements yet.</p>
        ) : (
          announcements.map((a) => {
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className={`rounded-full border px-2 py-0.5 text-[0.6875rem] font-bold ${LEVEL_CHIP[a.level] ?? LEVEL_CHIP.INFO}`}>{a.level}</span>
                      <span className={`rounded-full px-2 py-0.5 text-[0.6875rem] font-bold ${live ? "bg-emerald-500/15 text-emerald-600" : "bg-[var(--panel-strong)] text-[var(--ink-muted)]"}`}>
                        {live ? "Live" : a.isActive ? "Scheduled/expired" : "Inactive"}
                      </span>
                    </div>
                    <p className="mt-1.5 text-[0.875rem] font-bold text-[var(--ink)]">{a.title}</p>
                    <p className="mt-0.5 text-[0.8125rem] text-[var(--ink-muted)]">{a.body}</p>
