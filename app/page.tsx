    ...
    <>
      <main className="theme-blackgold relative flex min-h-screen flex-col overflow-hidden bg-[#050505]">
        ...
        <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-4 pt-10 pb-6 md:pt-12 md:pb-8">
          ...
          {/* Value prop */}
          <p className="fade-in mb-4 max-w-2xl text-balance text-[15px] font-semibold leading-snug text-white/85">
            Phone, laptop, tablet, and software repair in Kampala — written quotes, status updates, 30-day warranty.
          </p>

          {/* CTA row */}
          <div className="fade-in mb-5 flex flex-col gap-2 sm:mb-6 sm:flex-row sm:flex-wrap sm:gap-2.5">
            <Link
              href="/repair"
              className="flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-bold text-black shadow-[0_4px_18px_rgba(212,175,55,0.3)] sm:w-auto sm:py-2.5"
            ...
            <Link
              href="/address"
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/[0.05] px-5 py-2.5 text-sm font-semibold text-white/80 backdrop-blur-sm transition-colors hover:border-white/25 hover:bg-white/[0.09] hover:text-white sm:w-auto"
            >
          ...
          <div className="mb-5 space-y-3 sm:hidden">
            ...
            <div className="flex flex-wrap gap-2">
              {["Written quote", "No fix, no fee", "30-day warranty"].map((item) => (
                <span key={item} className="rounded-full border border-[var(--accent)]/30 bg-[var(--accent)]/[0.08] px-3 py-1 text-[11px] font-semibold text-[var(--accent)]">
                  {item}
                </span>
              ))}
            </div>
            ...
            {/* Steps */}
            <div>
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--ink-muted)]">How it works</p>
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-2 rounded-xl border border-white/8 bg-[#141414] px-4 py-3">
              {features.map((f) => (
                <div key={f.title} className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[var(--accent)]" />
                  <span className="text-xs font-semibold text-[var(--ink)]">{f.title}</span>
                </div>
              ))}
            </div>
          </div>

          {/* ── Desktop card grid — sm+ only ── */}
