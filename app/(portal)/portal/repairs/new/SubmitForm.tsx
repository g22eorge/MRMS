...
const input = "w-full rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-[0.8125rem] outline-none focus:border-[var(--accent)]/50";

function Submit() {
  const { pending } = useFormStatus();
  return (
    <button type="submit" disabled={pending} className="btn-premium rounded-lg px-4 py-2 text-[0.8125rem] text-white disabled:opacity-60">
  ...
  return (
    <form action={action} className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-[0.75rem] font-medium text-[var(--ink-muted)]">
          Device type
        ...
        <label className="text-[0.75rem] font-medium text-[var(--ink-muted)] sm:col-span-2">
