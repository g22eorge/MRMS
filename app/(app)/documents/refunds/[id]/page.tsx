...
export default async function RefundDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const secondary = (
    <>
      ...
      <Link href={`/api/refunds/${refund.id}`} className="btn-premium rounded-lg px-3 py-1.5 text-[0.75rem] font-bold">PDF</Link>
    </>
  );
          ...
          <div className={cardClass}>
            ...
            <div className="grid grid-cols-1 gap-4 p-4 min-[600px]:grid-cols-2">
              <div>
                ...
                <p className="mono text-[1.25rem] font-black tabular-nums">{formatMoney(refund.amount, currency)}</p>
              </div>
              <div>
              ...
              <div>
                <p className="mb-1 text-[0.6875rem] font-bold uppercase tracking-[0.12em] text-[var(--ink-muted)]">Refunded on</p>
                <p className="text-[0.8125rem] font-medium">{formatEATDate(refund.refundedAt)} · {formatEATTime(refund.refundedAt)}</p>
              </div>
              <div>
                <p className="mb-1 text-[0.6875rem] font-bold uppercase tracking-[0.12em] text-[var(--ink-muted)]">Reference</p>
                <p className="text-[0.8125rem] font-medium">{refund.reference || "—"}</p>
              </div>
              {refund.note && (
                <div className="min-[600px]:col-span-2">
                  ...
                  <p className="whitespace-pre-wrap text-[0.8125rem] text-[var(--ink-muted)]">{sanitizeText(refund.note)}</p>
                </div>
              )}
            </div>
          </div>
