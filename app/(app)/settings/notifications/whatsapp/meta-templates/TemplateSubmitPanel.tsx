...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="rounded-md border border-[var(--line)] bg-[var(--panel-strong)] px-2.5 py-1 text-[0.8125rem] font-semibold transition hover:bg-[var(--panel)] active:scale-95 shrink-0"
    >
      {copied ? "✓ Copied" : "Copy"}
    </button>
  );
...
export function TemplateSubmitPanel({ templates }: { templates: TemplateDefinition[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Requirements */}
      <div className="rounded-xl border border-blue-400/30 bg-blue-500/10 p-4 text-[0.8125rem] text-blue-900 dark:text-blue-300 space-y-1">
            ...
            <div key={t.metaName} className="rounded-xl border border-[var(--line)] bg-[var(--panel)] overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-5 py-3.5">
                <button type="button" onClick={() => setOpen(isOpen ? null : t.metaName)}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-[0.75rem] font-bold uppercase text-blue-700">{t.category}</span>
                      <span className="rounded-full bg-[var(--panel-strong)] px-2 py-0.5 text-[0.75rem] mono text-[var(--ink-muted)]">{t.language}</span>
                    </div>
                ...
                <div className="shrink-0 flex items-center gap-2">
                  ...
                  {st.state === "ok" && (
                    <span className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[0.8125rem] font-bold uppercase ${st.metaStatus === "APPROVED" ? "bg-emerald-500/10 text-emerald-700" : "bg-amber-500/10 text-amber-700"}`}>
                      {st.action === "updated"
                  ...
                  {st.state === "error" && (
                    <button onClick={() => submitOne(t)}
                      className="flex items-center gap-1.5 rounded-full bg-red-500/10 px-2.5 py-1 text-[0.8125rem] font-bold text-red-700 hover:bg-red-500/20 transition-colors"
                      title={st.message}>
                      ✕ Failed — retry
                    </button>
                  )}
              ...
              {isOpen && (
                <div className="border-t border-[var(--line)] px-5 py-4 space-y-4 bg-[var(--bg)]">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)] mb-1">Name</p>
                    ...
                    <div>
                      <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)] mb-1">Category</p>
                      <p className="text-sm font-semibold">{t.category}</p>
                    </div>
                    <div>
                      <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)] mb-1">Language</p>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)]">Body — submit to Meta as-is</p>
                  ...
                  {/* Variable mapping */}
                  <div>
                    <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)] mb-2">
                  ...
                  {/* Preview */}
                  <div>
                    <p className="text-[0.75rem] font-bold uppercase tracking-wide text-[var(--ink-muted)] mb-1.5">
                  ...
                  <div className="rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2 text-xs">
                    <span className="font-semibold text-[var(--ink-muted)] uppercase tracking-wide text-[0.75rem]">Trigger: </span>
