...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createRepairRequest({
      ...
      preferredDropoffDate: (body.preferred_dropoff_date || body.preferred_date) ? sanitizeOptionalText((body.preferred_dropoff_date as string) || (body.preferred_date as string)) : undefined,
      preferredDropoffTime: body.preferred_dropoff_time ? sanitizeOptionalText(body.preferred_dropoff_time as string) : undefined,
      dropoffNotes: body.dropoff_notes ? sanitizeOptionalText(body.dropoff_notes as string) : undefined,
      deliveryPersonName: body.delivery_person_name ? sanitizeOptionalText(body.delivery_person_name as string) : undefined,
      deliveryPersonPhone: body.delivery_person_phone ? normalizeUgandaPhone(body.delivery_person_phone as string) : undefined,
      deliveryCompany: body.delivery_company ? sanitizeOptionalText(body.delivery_company as string) : undefined,
      dispatchDate: body.dispatch_date ? sanitizeOptionalText(body.dispatch_date as string) : undefined,
      expectedArrivalTime: body.expected_arrival_time ? sanitizeOptionalText(body.expected_arrival_time as string) : undefined,
      deliveryTrackingReference: body.delivery_tracking_reference ? sanitizeOptionalText(body.delivery_tracking_reference as string) : undefined,
      deliveryFeeResponsibility: body.delivery_fee_responsibility ? sanitizeOptionalText(body.delivery_fee_responsibility as string) : undefined,
      deliveryNotes: body.delivery_notes ? sanitizeOptionalText(body.delivery_notes as string) : undefined,
      pickupAddress: body.pickup_address ? sanitizeOptionalText(body.pickup_address as string) : undefined,
      pickupLandmark: body.pickup_landmark ? sanitizeOptionalText(body.pickup_landmark as string) : undefined,
      preferredPickupDate: body.preferred_pickup_date ? sanitizeOptionalText(body.preferred_pickup_date as string) : undefined,
      preferredPickupTime: body.preferred_pickup_time ? sanitizeOptionalText(body.preferred_pickup_time as string) : undefined,
      alternateContactPerson: body.alternate_contact_person ? sanitizeOptionalText(body.alternate_contact_person as string) : undefined,
      alternateContactPhone: body.alternate_contact_phone ? normalizeUgandaPhone(body.alternate_contact_phone as string) : undefined,
      pickupNotes: body.pickup_notes ? sanitizeOptionalText(body.pickup_notes as string) : undefined,
    });
