...
/**
 * Bulk email for selected quotations.
 *
 ...
 * /api/quotations/bulk-email matched the [id] route instead, which exports
...
export async function POST(req: NextRequest) {
  ...
  for (const quotationId of ids) {
    ...
    const ok = await shareQuotationDocument({ orgId, quotationId, channel: "email" }).catch(() => false);
    if (ok) sent += 1;
