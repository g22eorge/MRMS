...
function typeIcon(type: string) {
  switch (type) {
    ...
    case "LEAD_WON":
      return (
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-yellow-500/15 text-yellow-400">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
    ...
    case "LEAD_LOST":
      return (
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-zinc-500/15 text-zinc-400">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            <path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/>
          </svg>
        </span>
    ...
    default:
      return (
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--accent)]/15 text-[var(--accent)]">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
  ...
  const panelStyle: React.CSSProperties = (() => {
    ...
    if (isMobile) {
      ...
      return {
        ...
        right: 8,
        width,
        maxHeight: `min(400px, calc(100dvh - ${rect.bottom + 24}px))`,
        zIndex: 9999,
      };
    }

    // Desktop: align the panel's right edge with the button's right edge.
    const desiredWidth = Math.min(20 * 16, vw - 16);
    ...
    return {
      ...
      right: safeRight,
      width: desiredWidth,
      maxHeight: `min(420px, calc(100dvh - ${rect.bottom + 24}px))`,
      zIndex: 9999,
    };
  })();
  ...
  return (
    <>
      ...
      <button
        ...
        className="relative flex h-9 w-9 items-center justify-center rounded-lg p-0 text-[var(--ink)]/80 transition hover:bg-[var(--panel-strong)] hover:text-[var(--ink)]"
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute right-0 top-0 flex h-[15px] min-w-[15px] items-center justify-center rounded-full bg-[var(--accent)] px-0.5 text-[9px] font-bold leading-none text-black ring-2 ring-[var(--panel)]">
      ...
      {isOpen && typeof document !== "undefined" && createPortal(
        <div
          ...
          className="overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--panel)] shadow-xl"
        >
          <style>{`
          ...
          {/* List */}
          <div className="divide-y divide-[var(--line)]/60 overflow-y-auto overscroll-contain" style={{ maxHeight: "calc(100% - 74px)" }}>
            {isLoading ? (
              <div>
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-3">
                    <div className="h-6 w-6 shrink-0 animate-pulse rounded-full bg-[var(--panel-strong)]" />
              ...
              notifications.map((n, i) => (
                <button
                  ...
                  className={`group w-full px-3 py-2 text-left transition-colors hover:bg-[var(--panel-strong)] ${
          ...
          {/* Footer — single prominent View all */}
          <div className="relative border-t border-[var(--line)]">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 flex h-6 w-6 items-center justify-center rounded-md text-[var(--ink-muted)]/60 transition hover:bg-[var(--panel-strong)] hover:text-[var(--ink)]"
            >
