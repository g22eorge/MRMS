          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="flex items-center gap-2 text-sm font-semibold">
                <span className={`inline-flex h-5 min-w-5 items-center justify-center rounded-full border px-1 text-[0.8125rem] font-bold ${actionMeta.chipClass}`}>
                  {actionMeta.icon}
                </span>
              ...
              <dl className="mt-2 grid gap-2 sm:grid-cols-2">
                {detailEntries.map(([key, value]) => (
                  <div key={key} className="rounded-md border border-[var(--line)] bg-[var(--panel)] px-2 py-1.5">
                    <dt className="text-[0.8125rem] uppercase tracking-[0.08em] text-[var(--ink-muted)]">{formatDetailKey(key)}</dt>
            ...
            {item.jobId ? <p className="mt-2 text-[0.75rem] font-semibold text-[var(--accent)]">View job →</p> : null}
          </>
        );
