        ...
        return (
          <div key={col.key} className="flex w-64 shrink-0 flex-col xl:w-[17rem]">
            <div className="mb-2 flex items-center gap-2 rounded-lg border border-[var(--line)] bg-[var(--panel-strong)] px-3 py-2">
              ...
              <span className={`rounded-full px-2 py-0.5 text-[0.75rem] font-bold ${col.badge}`}>{colJobs.length}</span>
            </div>

            <div className="flex flex-col gap-2">
              {colJobs.length === 0 ? (
                <div className="rounded-lg border border-dashed border-[var(--line)] py-8 text-center text-[0.8125rem] text-[var(--ink-muted)]/50">
                  No jobs
                </div>
              ) : (
                colJobs.map((job) => {
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1 space-y-1.5 p-2.5">
                        <div className="flex items-center justify-between gap-1">
                          <span className="text-[0.8125rem] font-bold tracking-tight text-[var(--ink)]">{job.jobNumber}</span>
                          {age > 0 ? (
                            <span
                        ...
                        <div className="flex items-center gap-1.5 text-[var(--ink-muted)]">
                          ...
                          <span className="truncate text-[0.8125rem] font-medium text-[var(--ink)]">{device}</span>
                        </div>

                        {(showClient && job.clientName) || job.assignedTo ? (
                          <div className="flex items-center justify-between gap-1 text-[0.75rem] text-[var(--ink-muted)]">
