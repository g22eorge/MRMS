...
export function RecordSummaryRail({
  ...
  return (
    <aside className="flex flex-col gap-4 lg:order-first">
      ...
      {resolvedParty && (
        <div className="rounded-xl border border-[var(--line)] bg-[var(--panel)] p-4">
          <p className="mb-2 text-[10.5px] font-bold uppercase tracking-[0.16em] text-[var(--ink-muted)]">
            {resolvedParty.title ?? "Client"}
          </p>
          <p className="text-[13px] font-semibold">{resolvedParty.name}</p>
          {resolvedParty.org ? <p className="text-[12px] text-[var(--ink-muted)]">{resolvedParty.org}</p> : null}
          {partyLines.length > 0 && (
